...
function PromptCardComponent({
  ...
  return <Card className="group cursor-pointer hover:shadow-lg transition-all duration-200 glass relative overflow-hidden" onClick={() => onPreview(prompt.id)}>
      <CardContent className={cn("p-6", variant === 'list' && "flex gap-4 sm:gap-6 items-center md:items-start")}>
        ...
        <div className={cn('relative', variant === 'list' ? 'w-28 sm:w-40 md:w-48 shrink-0' : 'mb-4')}>
          ...
          return <LazyImage src={thumbnailUrl} fullSrc={thumbnailUrl === fullImageUrl ? undefined : fullImageUrl} alt={`Preview do prompt ${prompt.title}`} className={cn(variant === 'list' ? 'h-20 sm:h-24 md:h-28 w-full' : 'aspect-video', 'rounded-lg')} cached={true} />;
        })()}
        </div>

        {/* Favorite indicator */}
        {prompt.isFavorite && <div className="absolute top-2 left-2 z-10">
            <Star className="w-4 h-4 text-favorite fill-current" />
          </div>}

        {/* Action buttons */}
        {variant === 'grid' && <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
          ...
          onToggleFavorite(prompt.id);
            ...
            <Button size="sm" variant="secondary" onClick={handleCopy} className="w-8 h-8 p-0">
              <Copy className="w-3 h-3" />
            </Button>
            {onGenerateImage && <Button size="sm" variant="secondary" onClick={e => {
          e.stopPropagation();
          onGenerateImage(prompt.id, cleanPromptContent(prompt.content || ''));
        }} disabled={isGeneratingImage} className="w-8 h-8 p-0" title="Gerar imagem com IA">
                {isGeneratingImage ? <div className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <Sparkles className="w-3 h-3" />}
              </Button>}
            {onEdit && <Button size="sm" variant="secondary" onClick={e => {
          e.stopPropagation();
          onEdit(prompt.id, {
            ...
            {onDelete && <Button size="sm" variant="destructive" onClick={e => {
          e.stopPropagation();
          onDelete(prompt.id);
        }} className="w-8 h-8 p-0">
        ...
        <div className={cn(variant === 'list' && "flex-1 min-w-0")}> 
          {/* Header */}
          {variant === 'list' ? <div className="mb-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    {prompt.number && <Badge variant="outline" className="font-mono text-xs">
                        #{prompt.number}
                      </Badge>}
                    <Badge className="bg-[hsl(var(--category-badge))] text-[hsl(var(--category-badge-foreground))] text-xs">
                      {toTitleCase(prompt.category)}
                    </Badge>
                    {prompt.subcategory && <Badge variant="outline" className="text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge className="bg-[hsl(var(--category-badge))] text-[hsl(var(--category-badge-foreground))] text-xs bg-stone-300">
                  {toTitleCase(prompt.category)}
                </Badge>
                {prompt.subcategory && <Badge variant="outline" className="text-xs">
                    {toTitleCase(prompt.subcategory)}
          ...
          {/* Tags */}
          {(prompt.styleTags?.length > 0 || prompt.subjectTags?.length > 0) && <div className="flex flex-wrap gap-1 mb-4">
              {prompt.styleTags?.slice(0, 3).map((tag: string) => <Badge key={tag} variant="secondary" className="text-xs">
                  {toTitleCase(tag)}
                </Badge>)}
              {prompt.subjectTags?.slice(0, 2).map((tag: string) => <Badge key={tag} variant="outline" className="text-xs">
                  {toTitleCase(tag)}
                </Badge>)}
              {(prompt.styleTags?.length > 3 || prompt.subjectTags?.length > 2) && <Badge variant="secondary" className="text-xs">
